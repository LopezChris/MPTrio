...
void LabGpioInterrupts::HandleInterrupt() {
    ...
    // Just clear everything and hope to god a nested handler wasn't
    //triggered
    // TODO: Figure out how to disable interrupts
    LPC_GPIOINT->IO0IntClr = 0xFFFFFFFF;
    LPC_GPIOINT->IO2IntClr = 0xFFFFFFFF;
}
bool LabGpioInterrupts::AttachInterruptHandler(uint8_t port, uint32_t
        pin, IsrPointer pin_isr, InterruptCondition condition) {
    ...
    if (port == 0) {
        ...
        switch(condition) {case kRisingEdge:
