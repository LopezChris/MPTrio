...
USBHOSTLITEDIR = $(APPLIBDIR)/USBHostLite

# List C source files here. (C dependencies are automatically generated.)
# use file-extension c for "c-only"-files
## Demo-Application:
SRC = main.c comm.c monitor.c ff_test_term.c
SRC += rtc_cal.c
#SRC += ff_test_term.c misc_test_term.c
## Chan's fat and my low-level drivers
SRC += $(FATFSDIR)/ff.c $(FATFSDIR)/option/ccsbcs.c
SRC += $(FATFSDIR)/diskio.c $(FATFSDIR)/spi_sd_lpc17xx.c  
SRC += $(FATFSDIR)/fattime.c 
...
# USBHostLite:
SRC += $(USBHOSTLITEDIR)/usbhost_lpc17xx.c
SRC += $(USBHOSTLITEDIR)/usbhost_ms.c
...
EXTRAINCDIRS  = $(NXPDRVINCDIR) $(CMSISDEVDIR) $(CMSISCOREDIR)
EXTRAINCDIRS += $(FATFSDIR) $(USBHOSTLITEDIR)
